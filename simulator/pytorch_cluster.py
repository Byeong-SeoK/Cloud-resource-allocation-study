...
filtered_data = data[data['task_name'].apply(lambda x: 'PyTorchWorker' in x)]
...
grouped_data.to_csv('pytorch_grouped_data.csv')


grouped_resource_usage = grouped_data.drop(columns=["inst_id", "group", "inst_id_count"])
grouped_resource_usage.to_csv("pytorch_grouped_resource_usage.csv")
...
cluster = [103, 25, 191, 11, 138, 66, 147, 93, 97, 12, 164]
count = 0
...
print('PyTorchWorker task resource clustering 정확도: ', 100-(count/len(min_diff_indices)*100), '%')
...
plt.title('PyTorchWorker task: Node & Resource Allocation Status')
