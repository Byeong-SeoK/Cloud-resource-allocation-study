...
filtered_data = data[data['task_name'].apply(lambda x: 'JupyterTask' in x)]
...
grouped_data = filtered_data.groupby(['inst_num', 'plan_cpu', 'plan_mem', 'plan_gpu']).agg({
    'inst_id': list,
    'group': list
})
...
grouped_data.to_csv('Jupyter_grouped_data.csv')


grouped_resource_usage = grouped_data.drop(columns=["inst_id", "group", "inst_id_count"])
grouped_resource_usage.to_csv("Jupyter_grouped_resource_usage.csv")
...
for _, row in test_set.iterrows():
    # 각 열과의 차이 계산
    diffs = np.abs(grouped_data[['plan_cpu', 'plan_mem', 'plan_gpu']] - row)
...
print('JupyterTask task resource clustering 정확도: ', (count/len(min_diff_indices)*100), '%')
...
plt.title('JupyterTask task: Node & Resource Allocation Status')
